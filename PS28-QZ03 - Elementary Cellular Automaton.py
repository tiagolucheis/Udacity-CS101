...
def cellular_automaton(string, pattern_number, generations):
    ...
    # makes a new string by appluing pattern to string
    # generations times
    for j in range(0, generations):
        new_string = ''
        
        for i in range(0, n):
            pattern = string[i-1] + string[i] + string[(i+1)%n] # uses modulus to retrieve the first element when the last one is reached
